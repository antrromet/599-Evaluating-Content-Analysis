...
def calculate_ratio():
    ...
    with open(merged_file_path) as lines:
        for line in lines:
            ...
            if 'X-Parsed-By' in json_line:
                ...
                added = False
                for item in parsers_list:
                    if item['name'] == content_type:
                        added = True
                        item['parsers'] = list(item['parsers']) + list(set(x_parsed_by) - set(list(item['parsers'])))
                if not added:
                    parsers_list.append({'name': content_type, 'parsers': x_parsed_by})
            file_size = json_line['file_size']
            ratio = len(json_line) / file_size
    ...
    print json.dumps(parsers_list, indent=4)
    return max_ratio
...
def write_bar_chart_files(bar_chart_data):
    for child in bar_chart_data:
        name = child['name']
        name = name.replace('/', '_')
        if not os.path.exists('metadata_retrieved_data'):
            os.makedirs('metadata_retrieved_data')
        with open('metadata_retrieved_data/' + name + '_data.tsv', 'w+') as data_file:
